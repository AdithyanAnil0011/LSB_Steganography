...
{
    int len = strlen(MAGIC_STRING); // length of the magic string to decode 
...
Status decode_secret_file_data(long secret_file_size, DecodeInfo* decInfo)
{
    ...
    char buffer[8];
    if (decInfo->fptr_secret == NULL)   //checks if output file was created
    { 
        printf("Error: File is not opened \n");
        return d_failure;
    }

    for (long i = 0; i < secret_file_size; i++)   // assume if hidden text is Hello then secret_file_size=5
    { 
        if (fread(buffer, 1, 8, decInfo->fptr_stego_image) != 8)
        {
        ...
        char data;   // to store decoded characters
        ...
        fwrite(&data, 1, 1, decInfo->fptr_secret);  // during each iteration each character is written ie 1st H then e,l,l,o
    }
    return d_success;
}
...
Status do_decoding(DecodeInfo* decInfo)
{
    ...
    decInfo->fptr_secret = fopen(output_file, "wb");
    ...
    long file_size;     // for storing the decoded file size
